...
class Geocodio {
  ...
  // Helper: Normalize distance mode
  normalizeDistanceMode(mode) {
    // API accepts: straightline, driving, haversine
    return mode;
  }
  ...
  addDistanceOptionsToParams(params, options) {
    if (options.mode) {
      params.mode = this.normalizeDistanceMode(options.mode);
    }
  ...
  addDistanceOptionsToBody(body, options) {
    if (options.mode) {
      body.mode = this.normalizeDistanceMode(options.mode);
    }
  ...
  addDistanceOptionsToGeocodeParams(params, distanceOptions) {
    ...
    if (distanceOptions.distanceMode) {
      params.distance_mode = this.normalizeDistanceMode(
        distanceOptions.distanceMode
      );
    }
