...
class Geocodio {
  ...
  list() {
    const create = listFile => {
      return axios.post(
        `https://api.geocod.io/v1.7/lists?file=${listFile}&api_key=${this.api_key}`
      );
    };

    const status = listId => {
      return axios.get(
        `https://api.geocod.io/v1.7/lists/${listId}?api_key=${this.api_key}`
      );
    };

    const all = () => {
      return axios.get(
        `https://api.geocod.io/v1.7/lists?api_key=${this.api_key}`
      );
    };

    const download = listId => {
      return axios.get(
        `https://api.geocod.io/v1.7/lists/${listId}?api_key=${this.api_key}/download`
      );
    };
