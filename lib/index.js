...
class Geocodio {
  ...
  list() {
    const createList = listFile => {
      return axios.post(listFile);
    };

    const listStatus = listId => {
      return axios.get(`https://api.geocod.io/v1.7/lists/${listId}`);
    };

    const getAllLists = () => {
      return axios.get(`https://api.geocod.io/v1.7/lists`);
    };

    const downloadList = listId => {
      return axios.get(`https://api.geocod.io/v1.7/lists/${listId}/download`);
    };
