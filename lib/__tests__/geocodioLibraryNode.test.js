...
describe("geocoder", () => {
  ...
  it("Can create a new list", () => {
    ...
    return geocoder.list.create("A New List").then(resp => {
  ...
  it("Can read link status", () => {
    ...
    return geocoder.list.status(1).then(resp => {
      expect(resp.results.id).toEqual(1);
    });
  });

  it("Can display all lists", () => {
    expect.assertions(1);

    return geocoder.list.download(1).then(resp => {
      expect(resp.results.id).toEqual(1);
    });
  });
  ...
  it("Can delete list", () => {
    ...
    return geocoder.list.delete(1).then(resp => {
      expect(resp.results.id).toEqual(null);
    });
  });
});
