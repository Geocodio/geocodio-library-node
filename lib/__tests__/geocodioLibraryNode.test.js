  ...
  it("Can create a new list, check status, download list and delete list", () => {
    ...
    let newList = geocoder.list
      ...
      .then(response => {
        ...
        geocoder.list.status(response.id).then(resp => {
          expect(resp.id).toEqual(response.id);

          geocoder.list.download(response.id, "newList").then(resp => {
            expect(resp.file.filename).toEqual("newList");

            geocoder.list.deleteList(response.id).then(resp => {
