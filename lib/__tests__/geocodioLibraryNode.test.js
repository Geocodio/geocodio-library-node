...
describe("geocoder", () => {
  ...
  it("Can create a new list, check status, download list and delete list", () => {
    ...
    let newList = geocoder.list.create(
    ...
    // Test created list against filename
    let createTest = newList.then(response => {
      expect(response.file.filename).toEqual("sample_list.csv");
    });

    // Isolate listId from newly create list
    let listId = newList.id;

    // Query status using listId
    let statusTest = geocoder.list.status(listId).then(response => {
      expect(response.id).toEqual(listId);
    });

    // Download using listId
    let downloadTest = geocoder.list.download(listId).then(response => {
      expect(response.id).toEqual(listId);
    });

    // Delete using listId
    let deleteTest = geocoder.list.deleteList(listId).then(response => {
      expect(response.id).toEqual(null);
    });

    // Returns Test Results
    return createTest && statusTest && downloadTest && deleteTest;
  });
});
