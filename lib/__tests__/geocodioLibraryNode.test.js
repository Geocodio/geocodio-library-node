...
describe("geocoder", () => {
  ...
  it("Can create a new list", () => {
    ...
    return geocoder.list.createList("A New List").then(response => {
      expect(response.results.length).toEqual(1);
    });
  });

  it("Can read link status", () => {
    expect.assertions(1);

    return geocoder.list.listStatus(1).then(response => {
      expect(response.results.id).toEqual(1);
    });
  });
  ...
  it("Can download a list", () => {
    ...
    return geocoder.list.downloadList(1).then(response => {
      expect(response.results.id).toEqual(1);
    });
  });
  ...
  it("Can delete list", () => {
    ...
    return geocoder.list.deleteList(1).then(response => {
      expect(response.results.id).toEqual(null);
    });
  });
});
